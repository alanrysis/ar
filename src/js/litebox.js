		...
		liteBox.prototype = {
			...
			populateLitebox: function(link) {
				// Variables
					var	$this = this,
						href = link.attr('href'),
						hrefInline = link.attr('href'),
						$currentContent = $('.litebox-content');
						
					var inlineContentHref = hrefInline.substring( hrefInline.search('#'), (hrefInline.length + 1) );
				...
				// Process
					if (href.match(/\.(jpeg|jpg|gif|png|bmp)/i) !== null) {
						...
						$img.error(function() {
							...
							var $iframe = $('<iframe>', {
								...
								'hspace': '0',
								'scrolling': 'auto',
								'class': 'litebox-content litebox-container-mauthor',
								'allowfullscreen': ''
							});
						...
						if (videoURL[1] == 'youtube')
							src = 'http://www.youtube.com/embed/' + videoURL[5] + '?autoplay=1&rel=0';
						
						if (videoURL[1] == 'youtu')
							src = 'http://www.youtube.com/embed/' + videoURL[3] + '?autoplay=1&rel=0';
						...
						if (src) {
							var $iframe = $('<iframe>', {
								...
								'scrolling': 'no',
								'allowfullscreen': '',
								'class': 'litebox-content litebox-content-video',
					...
					} else if (inlineContentHref.substring(0, 1) == '#') {
						if ($(inlineContentHref).length) {
							$html = $('<div>', { 'class': 'litebox-content litebox-inline-html' });

							$html.append($(inlineContentHref).clone());
				...
				// Inline
					if (typeof $html != 'undefined' && $('.litebox-inline-html').outerHeight() < $container.height())
						$('.litebox-inline-html').css({
							...
							'top': '50%',
							'margin-left': '-' + ( $('.litebox-inline-html').outerWidth()) / 2 + 'px',
							'left': '50%'
						});
			},
